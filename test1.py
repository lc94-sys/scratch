...
class RetrievalService:
    ...
    def search(
        ...
        Args:
            ...
            search_type: 'hybrid' (default), 'vector', or 'text'
                - hybrid: Combines vector + BM25 (replaces old FAISS + BM25)
                - vector: Vector search only (k-NN)
                - text: Text search only (BM25)
        """
        ...
        if search_type == 'vector':
            ...
            # Text only (BM25)
            return self.opensearch_client.text_search(
                query_text=query,
                entitlement=entitlement,
                org_id=org_id,
                tags=tags,
            ...
            # Hybrid (vector + BM25) - DEFAULT
            query_embedding = self.embedding_model.get_embedding(query)
            return self.opensearch_client.hybrid_search(
                query_text=query,
                query_embedding=query_embedding,
                entitlement=entitlement,
                org_id=org_id,
